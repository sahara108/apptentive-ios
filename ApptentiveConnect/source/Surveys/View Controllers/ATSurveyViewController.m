...
{
	for (NSInteger i = _survey.questions.count - 1; i >= 0; i--) {
		ATSurveyQuestion *q = _survey.questions[i];
...
- (NSInteger)numberOfSectionsInTableView:(UITableView *)aTableView {
	return [_survey activeQuestionCount] + 1;
}

- (NSInteger)tableView:(UITableView *)aTableView numberOfRowsInSection:(NSInteger)section {
	if (section < [_survey activeQuestionCount]) {
		NSUInteger result = 0;
		...
		return result;
	} else if (section == [_survey activeQuestionCount]) {
		return 1;
	}
	return 0;
}
...
- (UITableViewCell *)tableView:(UITableView *)aTableView cellForRowAtIndexPath:(NSIndexPath *)indexPath {
	...
	if (indexPath.section == [_survey activeQuestionCount]) {
		...
		return buttonCell;
	} else if (indexPath.section >= [_survey activeQuestionCount]) {
		return nil;
	}
	ATSurveyQuestion *question = [self questionAtIndexPath:indexPath];
...
- (NSString *)tableView:(UITableView *)aTableView titleForFooterInSection:(NSInteger)section {
	if (section == [_survey activeQuestionCount] && _errorText != nil) {
		return _errorText;
	}
	return nil;
}
...
- (void)scrollToBottom {
	if (self.tableView) {
		NSIndexPath *path = [NSIndexPath indexPathForRow:0 inSection:[_survey activeQuestionCount]];
...
- (ATSurveyQuestion *)questionAtIndexPath:(NSIndexPath *)indexPath {
	...
	ATSurveyQuestion *q = [[_survey questions] objectAtIndex:indexPath.section];
	if ([q isHiddenQuestion]) {
		return nil;
	}
